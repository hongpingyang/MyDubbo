...
import com.hong.py.config.ArgumentConfig;
import com.hong.py.config.MethodConfig;
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
        ...
        if ((id == null || id.length() == 0) && required) {
            ...
            if (generatedBeanName == null || generatedBeanName.length() == 0) {
                //如果protocol的name没有配置，则默认为dubbo，其他的取Interface名称
            ...
            int counter = 2;
            //如果已经在容器里注册过了，则直接再名称后加上最后注册的序号+1
        ...
        for (Method setter : beanClass.getMethods()) {
            ...
            if (name.length() > 3 && name.startsWith("set")
                ...
                //获取get方法或者is方法 new Class[0]经常用来告诉反射的getMethod函数找到一个无参数的函数。
                try {
